//! The wscript standard library (PRD §7).
//!
//! Every module is **opt-in at embed time** (capability-style — nothing is
//! ambient): `Context::new().module(wscript_std::math())`. Each module is also
...
/// All enabled stdlib modules (what `wscript run` registers by default).
/// `script_args` feeds `process::args()`.
pub fn all_modules(script_args: Vec<String>) -> Vec<wscript_core::Module> {
    #[allow(unused_mut)]
