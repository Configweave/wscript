...
**Upstream version:** `0.2.0`. If the real upstream has moved past this, the skill may be stale — bump `topic.version` and re-verify (see the update workflow).
...
- `$ARGUMENTS`: The wscript topic, stdlib function, CLI subcommand, or embedding API to look up. How to determine: Take it from the user's request — e.g. the language feature, method, module, or `wscript` subcommand they asked about. If empty, summarise the reference and ask what they need.
- `$1`: Optional area to scope the answer to: `language`, `stdlib`, `cli`, or `embedding`. How to determine: Infer from the question; default to searching all areas when unset.
...
- Suggest unsupported features (borrow checker/`&`/lifetimes, generic structs/enums/traits, bitwise ops, exceptions, async, threads sharing values, format specs inside interpolation holes) as if they exist.
...
Build with `cargo build --release` (binary at `target/release/wscript`). Execution starts at `fn main()`; its `int` return becomes the exit code (`unit` means 0); the extension is `.wscript`.

```rust
// hello.wscript
fn main() -> int {
    let who = "wscript"
    println("Hello, {who}!")     // string interpolation
    0
}
```

`wscript run hello.wscript` executes with the full stdlib; `check` compiles only (honors `wscript.toml`/`.wscripti`); `repl` and `lsp` cover exploration and editors — see [the CLI](references/fact_cli_commands.md). Embedding is five steps: `Module` → `Context` → `ctx.compile` (all type errors surface here) → one `Vm` per thread → `vm.call` / `unit.fn_handle`; see [Embedding](references/concept_embedding.md) and the **Embed wscript in a Rust application** process.
...
- [Not in wscript (by design)](references/fact_not_in_v1.md)
...
- [time module](references/fact_time_functions.md)
- [regex module](references/fact_regex_functions.md)
...
- [Benchmarks vs Lua / Rhai / Rune](references/fact_benchmarks.md)
