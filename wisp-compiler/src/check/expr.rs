...
use wisp_core::defs::{self, DefId, DefKind, VariantKind};
...
impl<'a> Checker<'a> {
    ...
    fn expect_bool(&mut self, t: &Type, span: Span, what: &str) {
        ...
        self.error_help(
            ...
            "wisp has no truthiness: write an explicit comparison",
        );
    }
    ...
    fn module_item_lookup(
        ...
        mod_idx: usize,
        name: &str,
    ) -> Option<Result<(FnSig, u32), (Type, wisp_core::bytecode::Const)>> {
        ...
        match self.module_item_lookup(mod_idx, &item.name) {
            ...
            None => {
                ...
                self.error_help(
                    ...
                    "check the module's `.wispi` interface for available items",
                );
                Type::Error
            }
        }
    ...
    fn check_named_method(&mut self, e: &Expr, def: DefId, name: &Ident, args: &[Expr]) -> Type {
        ...
        if name.name == "clone" && self.derives.get(&def).is_some_and(|d| d.clone) {
            ...
            self.out
                .methods
                .insert(e.id, MethodRes::Builtin(wisp_core::Builtin::DeepClone));
