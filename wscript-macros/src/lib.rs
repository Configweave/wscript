//! `#[derive(Script)]` (PRD §6.2): expose a Rust type to wscript scripts.
//!
...
//!
//! Generated code references the `wscript` umbrella crate by default; inside
//! crates that depend on `wscript-core` directly, use
//! `#[script(crate_path = "wscript_core")]`.
...
struct Options {
    opaque: bool,
    /// Path to wscript-core (default goes through the `wscript` umbrella).
    core: TokenStream2,
...
fn expand(input: DeriveInput) -> syn::Result<TokenStream2> {
    if !input.generics.params.is_empty() {
        return Err(syn::Error::new_spanned(
            &input.generics,
            "#[derive(Script)] types must be monomorphic (wscript v1 has no user generics)",
        ));
    }
    ...
    let Fields::Named(fields) = &data.fields else {
        return Err(syn::Error::new_spanned(
            ...
            "#[derive(Script)] data structs need named fields \
             (tuple structs are not representable in wscript; use #[script(opaque)])",
        ));
    };
